...
| | CC Switch | 9router |
|------|-----------|---------|
| **核心功能** | 多 Agent 工具配置管理 | 智能路由 + Token 压缩 |
| **Token 节省** | 无 | RTK 节省 20-40% |
| **自动 Fallback** | 无 | 多级自动降级 |
| **接入方式** | 同步目标工具配置 | 本地代理服务 |
| **免费模型** | 只管理供应商配置 | 提供免费 fallback 连接方式 |
| **适用场景** | 多个 Agent CLI 的配置切换 | 请求路由、压缩和自动回退 |
...
在设置中修改 API Base URL：
```
https://api.openai.com/v1  →  http://localhost:20128/v1
```
...
将 API Base URL 指向 `http://localhost:20128/v1`，API Key 填 `9router` 或你的实际密钥。
