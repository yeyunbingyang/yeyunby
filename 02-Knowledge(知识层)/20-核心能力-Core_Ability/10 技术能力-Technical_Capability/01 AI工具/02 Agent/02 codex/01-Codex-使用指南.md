...
|---|---|---|---|---|
|Browser|让Codex操作**内置浏览器**|一般用于前端的自动化测试|1. 自动登录网站<br>    <br>2. 点击按钮<br>    <br>3. 填写表单<br>    <br>4. 翻页截图<br>    <br>5. 确认页面功能和样式是否正常|处理公开网页、本地预览、无需登录的页面，则优先使用 Codex 自带 Browser|
|Computer Use|让codex多一双“眼睛”和“鼠标”，**能操控所有电脑软件**|1. 命令行或插件不够用的场景<br>    <br>2. 完全依靠图形界面交互的场景|1. 测试桌面 App<br>    <br>2. 复现界面 Bug<br>    <br>3. 检查导入导出流程<br>    <br>4. 打开音乐软件搜歌<br>    <br>5. 使用微信界面发消息|目前仅限mac系统|
|Chrome|codex控制**真实的谷歌浏览器**，并在后台执行浏览器操作|适合处理“必须登录真实网站才能完成”的浏览器任务|1. 打开 Gmail 查邮件<br>    <br>2. 进公司后台改资料<br>    <br>3. 跨多个网页整理资料|不会直接接管你正在用的窗口，导致影响你正常使用谷歌浏览器|
...
|------|------|-----------|------|
| 默认 | 项目内读写 | ❌ 需弹窗 | ❌ 需弹窗 |
| 自动审查 | AI 判断安全 | ✅ 常见命令自动 | ✅ 安全来源 |
| 完全访问 | 无限制 | ✅ 无条件 | ✅ 无条件 |

当前：`sandbox_mode = "workspace-write"`（自动审查级别）。
...
共 16 个详见 [[02-插件与技能系统]]。核心：browser(-use)、chrome、computer-use、documents、spreadsheets、presentations、github、visualize。

### 记忆系统

| 层级 | 范围 | 召回 |
|------|------|------|
| 全局 AGENTS.md | 所有对话 | 每次 100% |
| 项目级 AGENTS.md | 当前项目 | 每次 100% |
| 自动记忆 | 跨对话 | RAG 按需 |
...
| 查看状态 | `/status` |
| 新线程 | `/thread new` |
| 安装 Skill | "帮我装 xxx Skill" |
| 创建 Skill | "刚才这套做成 Skill" |
| 查看插件 | 左侧插件面板 |
...
- **沙箱**：elevated（Windows 提权模式），权限级别相当于自动审查
- **引导模式**：queue（排队执行，不打断当前任务）

### 项目（Project）要点

- 项目 = Codex 可自由读写的本地文件夹
- 同一项目可开多个对话，共享文件夹权限，各有独立上下文
- 当前信任项目：x:\kms\yeyunby、:\code\yudao-mini、:\project
- 权限绑定项目而非对话
...
ode_repl 是核心 MCP——持久化 Node.js 运行时，驱动浏览器自动化和脚本执行。安装新 MCP 可直接把仓库链接发给 Codex，自动引导完成配置授权。

### Automations（自动化）

- 自动化面板新建 / 对话直接说"帮我创建一个自动化任务"
- 绑定 Skill 后触发的是"微型 Agent 实例"（有完整上下文和工具链）
- 典型场景：每周一早上 9 点自动产出一篇 GitHub 热门项目推荐并发布到飞书群
...
| 自动化管理 | /automations |
| 安装 Skill | "帮我装 xxx Skill" |
| 创建 Skill | "刚才这套流程帮我做成 Skill" |
| 模型切换 | 修改 config.toml 或 cc-switch |
| 清空对话 | /clear 或 Ctrl+L |

### 引导（Queue）与 Fork

- **引导**：长任务执行中输入修正指令 → 排队 → 下次工具调用时自动带上，不打断当前执行
- **Fork**：从任意 AI 回复点分叉新对话，保留上下文
...
| 扩展 | 3 层（Skill→Agent-Skill→Plugin） | 1 层（.claude/skills/）|
| Plugin | ✅ 有 | ❌ 无 |
| 浏览器操控 | ✅ browser-use + chrome | ❌ 需额外配置 |
| 生图 | ✅ Image-2 | ❌ |
| 手机远程 | ✅ ChatGPT App | ❌ |
| 计划模式 | ✅ 内置 | ❌ |
| 沙箱 | ✅ 有 | ❌ |
...
所有 51 张飞书截图已下载到本地 images-wx/ 目录。
