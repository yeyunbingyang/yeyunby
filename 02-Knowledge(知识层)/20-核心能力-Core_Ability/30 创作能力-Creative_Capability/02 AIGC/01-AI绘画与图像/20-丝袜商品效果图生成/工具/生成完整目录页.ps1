...
$sourceCropTop = [int]($sourceCellHeight * 0.42)
...
for ($index = 0; $index -lt 6; $index++) {
    ...
    $sourceY = $row * $sourceCellHeight + $sourceCropTop
    $sourceWidth = if ($column -eq 2) { $source.Width - $sourceX } else { $sourceCellWidth }
    $sourceCellBottom = if ($row -eq 1) { $source.Height } else { ($row + 1) * $sourceCellHeight }
    $sourceHeight = $sourceCellBottom - $sourceY
