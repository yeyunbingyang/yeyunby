...
related:
  - "[[Core_Ability-MOC]]"
summary: "AI音乐与音频知识域按 01–05 能力组织：基础知识、音乐、TTS、SFX 与音频处理；模型/工具直接融入对应能力，实验/工作流/Prompt/资产继续向 07–10 沉淀。"
---
...
> 原 `06-工具与模型` 已取消。模型和工具现在跟随它们主要服务的能力归档，不再建立第二套横向工具树。
...
3. 一个项目横跨多个能力时，只保留一个主技术卡，其他目录通过双链引用。
4. 未实测内容标记为“调研结论”，不能写成“本机已验证”。
5. 动态信息写 `verified` / `review_after`。
6. 正式实验进入 08，成熟流程再升格到 07。
