...
![[Pasted image 20260630153855.png]]
...
`[https://www.cctv.com/#SUBD1605080062959435](https://www.cctv.com/#SUBD1605080062959435)`，其中的`#SUBD1605080062959435`就是 hash 值，hash 值只在客户端（如浏览器）中使用，是不会带给服务器的，所以使用 hash 模式时，**不存在**刷新 404 问题。
...
```
location /dev/ {
-  proxy_pass http://sph-h5-api.atguigu.cn;   # 错误：会保留 /dev/
+  proxy_pass http://sph-h5-api.atguigu.cn/;  # 正确：移除 /dev/
}
```
