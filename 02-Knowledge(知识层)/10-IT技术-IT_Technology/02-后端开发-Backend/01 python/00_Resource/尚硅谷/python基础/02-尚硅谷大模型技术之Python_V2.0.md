---
title: 尚硅谷大模型技术之Python
domain: IT_Technology
tags: [Python]
status: 草稿
created: 2026-06-14
source: "尚硅谷大模型技术之Python V2.0"
summary: ""
---
# 尚硅谷大模型技术之Python
...
| %= | 模赋值 | a %= 2，等同于a = a % 2 |
| **= | 幂赋值 | a **= 2，等同于a = a ** 2 |
| := | 海象运算符，在表达式中同时进行赋值和返回赋值的值。Python3.8 版本新增 | num1 = 20print((num2 := 3**2) &gt; num1)print(num2) |
...
| 反码 | 等于原码00000111 | 原码符号位不变，其余位取反11111000 |
| 补码 | 等于原码00000111 | 反码的基础上+111111001 |
...
方法解析顺序（mro—Method Resolution Order）。可使用 类名.__mro__ 访问类的继承链来查看方法解析顺序。super() 不是简单的 “调用父类”，而是基于方法解析顺序（MRO） 调用 “当前类在 MRO 链中的下一个类”。
...
        # 直接调用 Parent1 的 __init__ 方法
        Parent1.__init__(self, value1)
        # 直接调用 Parent2 的 __init__ 方法
...
```python
from . import circle  或者 from day09.graphic import circle
```
