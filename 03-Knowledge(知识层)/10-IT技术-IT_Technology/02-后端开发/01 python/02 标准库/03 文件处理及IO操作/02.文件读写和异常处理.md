...
status: 草稿
summary: 文件打开/关闭/读写、异常处理机制、上下文管理器、二进制文件操作、os.walk遍历、文件拷贝
tags: [Python, 文件操作, IO]
created: 2026-06-12
...
![[Pasted image 20260614013534.png]]

### 文件的基本概念

文件是存储在磁盘上的数据集合，可包含文本、图像、音频、视频、程序代码等。文件系统通过文件名和文件路径来定位和管理文件。将内存中的数据写到磁盘文件的过程称为**持久化**。

#### 文件的分类

| 类型 | 说明 | 常见格式 |
|------|------|---------|
| **纯文本文件** | 有统一编码，可视为存储在磁盘上的长字符串 | `.txt`, `.py`, `.csv`, `.json` |
| **二进制文件** | 无统一编码，直接由 0/1 组成 | `.jpg`, `.png`, `.avi`, `.mp3` |

纯文本文件常用编码：ASCII、ISO-8859-1、GB2312、GBK、UTF-8、UTF-16 等。

#### 文件的路径

| 类型 | 说明 | 示例 |
|------|------|------|
| **相对路径** | 从当前位置到指定位置的路径，`./` 表示当前，`../` 表示上级 | `./hello.py`, `../data/test.txt` |
| **绝对路径** | 从根目录到指定位置的完整路径 | `E:/Hello/hello.py`, `/home/user/test.txt` |
...
![[Pasted image 20260614013903.png]]

下图展示了如何根据程序的需要来设置`open`函数的操作模式。

![[Pasted image 20260614013915.png]]
...
open(
    ...
    mode="r",      # 文件打开模式
    buffering=-1,  # 缓冲策略
    encoding=None, # 文本编码方式，一般用 utf-8
    errors=None,   # 编解码错误处理
    newline=None,  # 换行符处理
    closefd=True,  # 文件描述符关闭行为
    opener=None,   # 自定义开启器
)
```
...
readline([size]) 一次读取一行，可通过 size 限制单次读取字节数：

```python
f = open('test.txt', 'rt')
print(f.readline())      # 读取第一行
print(f.readline(1))     # 从第二行读取 1 个字节
print(f.readline(1))     # 继续读取 1 个字节
f.close()
```

#### readlines 按行读取为列表

readlines([size]) 读取所有行返回列表，size 参数控制累计字节总量（实际可能略多）：
...
os.walk() 自顶向下递归遍历目录树，返回三元组 (dirpath, dirnames, filenames)：
...
|------|------|
| top | 根目录路径 |
| topdown=True | 默认自顶向下；False 则自底向上 |
| onerror=None | 出错时的回调函数 |
| followlinks=False | 是否通过软链接访问目录 |
...
copy_file_chunked('D:/mv.png', 'E:/mv.png')
```
