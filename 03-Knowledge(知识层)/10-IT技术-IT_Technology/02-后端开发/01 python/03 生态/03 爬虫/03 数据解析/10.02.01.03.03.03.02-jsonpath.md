...
回顾 [[10.02.01.03.03.03.01-数据提取基础]] 中的「骨骼·肌肉·皮肤」三文件模型：
...
- **肌肉层数据**：jsonpath 是 Ajax/XHR JSON 数据的标配工具，配合 [[10.02.01.03.03.03.01-数据提取基础]] 的三文件模型使用
...
- [[10.02.01.03.03.03.01-数据提取基础]] — 响应内容分类 + 三文件模型：jsonpath 对应「肌肉层」Ajax JSON 数据
- [[10.02.01.03.03.03.03-lxml模块与XPath]] — HTML/XML 提取用 XPath（骨骼层），JSON 用 jsonpath（肌肉层）——语法高度相似
